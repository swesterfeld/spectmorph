...
{
  static HackVoiceStatus hack_voice_status;

  hack_voice_status.on_synth_notify_event (ne);
  if (hack_voice_status.changed)
    {
      ...
      x_voice_values = hack_voice_status.get_values (*x_morphing);
      y_voice_values = hack_voice_status.get_values (*y_morphing);
      update();

      hack_voice_status.changed = false;
    }
}
